...
def GameRec(request):
    ...
    if request.method == 'POST':
        ...
        if(gameRecClass.objects.filter(UserName=user).count() != 0):
            ...
            serializer = gameRecClass(snippets, many=True)
        ...
        rec = request.POST['Record']
        if(rec==""):
            if(postClass.objects.filter(GameName = Game).count() != 0):
            ...
            if(postClass.objects.filter(GameName = Game).filter(Record = rec).count() != 0):
